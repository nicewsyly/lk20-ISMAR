#ifndef _AFFINE_ICNTD_H
#define _AFFINE_ICNTD_H
#include <opencv2/opencv.hpp>
#include <iostream>
#include "tools.hpp"

cv::Matx<float,2,3> icntdupdate_step(const cv::Matx<float,2,3>& warp_p,cv::Mat delta_p);
typedef struct icntdfit_
{
    cv::Matx23f warp_p;
    float rms_err;
}icntdfit;
...
std::vector<icntdfit> affine_ic_nt_d(const cv::Mat& img,const cv::Mat& templ,const cv::Matx<float,2,3>& p_init,const int n_iters,const int step_size,cv::Matx<T,M,N>& warp_p,const std::vector<cv::Point2f>& templ_pts)
{
    cv::Mat aimg;
    if(img.channels()==1)
        aimg=img.clone();
    ...
    for(int i=0;i<np;++i)
    {
        ...
        //    cv::Mat qy=dwdp(cv::Rect(j*width,height,width,dwdp.rows-height));
            H_extra(cv::Rect(i*width,i*height,width,height))=tx2.mul(tx)+ty2.mul(ty);
        //}
    } 
    ...
    std::vector<icntdfit> fita(n_iters);
    for(int ni=0;ni<n_iters;++ni)
    {
        ...
        for(int i=0;i<np;++i)
        {
                cv::Mat sub_h=H_extra(cv::Rect(i*width,i*height,width,height)).clone();
                cv::Mat mul_h=(sub_h.mul(err_img)); 
                H_nt.at<float>(i,i)=cv::sum(mul_h).val[0];
        }
        H_nt+=H;
        ...
        warp_p=icntdupdate_step(warp_p,delta_p);
